...
import { BigNumberVizProps, BinaryQueryObjectFilterClause, EventHandlers, SMART_DATE_VERBOSE_ID } from './types';
...
class BigNumberVis extends PureComponent<BigNumberVizProps> {
    ...
    if (hasThresholdColorFormatter) {
      colorThresholdFormatters!.forEach(formatter => {
        const formatterResult =
          // @ts-ignore
          bigNumber && formatter.column !== 'сomparison_lag_value'
  ...
  renderSubheader(maxHeight: number) {
    ...
    let text = subheaderFromCondition ?? subheader;
    if (bigNumber === null) {
    ...
    if (hasThresholdColorFormatter) {
      colorThresholdFormatters!.forEach(formatter => {
        const formatterResult =
          // @ts-ignore
          timeLagNumber && formatter.column === 'сomparison_lag_value'
            ? formatter.getColorFromValue(timeLagNumber * 100)
            : false;
        if (formatterResult) {
          numberColor = formatterResult;
        }
